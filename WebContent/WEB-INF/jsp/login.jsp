...
// 判断是登录账号和密码是否为空
function check(){
    var usercode = $("#usercode").val();//<input id="usercode" type="text" name="usercode" />
    var password = $("#password").val();//<input id="password" type="password" name="password" />
    if(usercode=="" || password==""){
    	$("#message").text("账号或密码不能为空！");//<span id="message">${msg}</span>
        return false;
    }  
    return true;
}
   ...
   <td class="login_msg" width="400" align="center">
	 ...
	 <fieldset style="width: auto; margin: 0px auto;">
		...
		<form action="${pageContext.request.contextPath }/login.action" 
			                       ...
			                       <!-- onsubmit调用check方法，在上面的js代码中,在表单提交时，进行验证 check()不通过后会返回false，
		 因为onsubmit属性就像是<form>这个html对象的一个方法名，其值（一字符串）就是其方法体，默认返回true，所以还是相当于验证通过  -->
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br /><br />
